...
void cl::preprocess_include(cl::Context& context, std::string& code, std::string_view parent_filepath) noexcept
{
...
void cl::preprocess_definition(cl::Context& context, std::string& code) noexcept
{
    std::regex rx_pdef;
    std::smatch matches;
}

void cl::preprocess_macro_definition(cl::Context& context, std::string& code) noexcept
{
    ...
    while (std::regex_search(code, matches, rx_mcro))
    {
        ...
        cl::Macro macro{ name, args_delim, mcode };
        
        if (std::find_if(context.defined_macros.begin(), context.defined_macros.end(), [&](cl::Macro& m)
            {
...
void cl::preprocess_macro_invokation(cl::Context& context, std::string& code) noexcept
{
    ...
    while (std::regex_search(code, matches, rx_invk))
    {
        ...
        cl::Macro macro{ name, args_delim, "" }, defined_macro;
...
void cl::preprocess(std::string& code, std::string_view parent_filepath) noexcept
{
    cl::Context context{ {}, {} };
    
    cl::preprocess_include(context, code, parent_filepath);
    cl::preprocess_definition(context, code);
    cl::preprocess_macro_definition(context, code);
    cl::preprocess_macro_invokation(context, code);
}
